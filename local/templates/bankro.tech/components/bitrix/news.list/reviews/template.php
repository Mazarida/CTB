...
<div class="swiper-wrapper">
    <?foreach($arResult["ITEMS"] as $arItem):?>
        ...
        <div id="<?=$this->GetEditAreaId($arItem['ID']);?>" class="swiper-slide information-slider__box slider-reviews__box">
            <?if ($arParams["DISPLAY_PREVIEW_TEXT"] != "N" && $arItem["PREVIEW_TEXT"]):?>
                <span class="slider-reviews__text"><?=$arItem["PREVIEW_TEXT"]?></span>
            <?endif;?>

            <?if ($arParams["DISPLAY_PICTURE"] != "N" && is_array($arItem["PREVIEW_PICTURE"])):?>
                <div class="slider-reviews__logo"><img src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$arItem["PREVIEW_PICTURE"]["ALT"]?>"></div>
            <?endif;?>

            <?if ($arParams["DISPLAY_NAME"] != "N" && $arItem["NAME"]):?>
                <span class="slider-reviews__name-compani"><?=$arItem["NAME"]?></span>
            <?endif;?>
        </div>
    <?endforeach;?>
</div>
